...
class EstudianteController extends Controller
{
    ...
    public function showNotas(int $id)
    {
        ...
        if (isset($seccion[0])) {
            if (in_array($id, session()->get('ArregloSecciones'))) {
                ...
                return view('sitioEstudiante.showNotas', compact(
                    'seccion',
                    'materias',
                    'notas11',
                    'notas12',
    ...
    public function showPerfil()
    {
        if (!session()->has('estudiante')) {
            ...
            $user = session()->get('estudiante');

            $secciones = DB::table('detalleseccionestudiante')
                ->join('secciones', 'detalleseccionestudiante.idSeccion', '=', 'secciones.idSeccion')
                ->join('añoescolar', 'secciones.idAño', '=', 'añoescolar.idAño')
                ->where('idEstudiante', '=', $user[0]->NIE)
                ->orderBy('detalleseccionestudiante.idDetalle')
                ->get();

            return view('sitioEstudiante.showPerfil', compact('user', 'secciones'));
        }
    }
    ...
    public function cambiarContraseñaEstudiante(Request $request)
    {
        ...
        if ($user[0]->contraseña == Hash('SHA256', $passwordActual)) {
            ...
            try {
                ...
                return to_route('estudiante.cambiarContraseñaEstudent')->with('exitoCambiar', 'La contraseña del usuario ha sido cambiada con éxito');
            } catch (Exception $e) {
                return to_route('estudiante.cambiarContraseñaEstudiante')->with('errorCambiar', 'La contraseña del usuario no sido cambiada con éxito');
            }
        } else {
            return to_route('estudiante.cambiarContraseñaEstudent')->with('errorCambiar', 'La contraseña del usuario no sido cambiada con éxito');
        }
    }
}
