        ...
        <form method="POST" action="{{route('profesor.insertarNotas')}}">
            ...
            <table class="table table-bordered align-middle justify-content-center data-table">
                ...
                <tbody>
                    ...
                    <input type="number" value="{{$grupo[0]->idDetalle}}" name="idDetalleMateria" hidden>
                    <input type="text" value="{{$periodo[0]->idPeriodo}}" name="idPeriodo" hidden>
                    @foreach ($estudiantes as $students)
                    <tr>
                        <td scope="col">{{$students->nombres}} {{$students->apellidos}}</td>
                        <td>
                            <div class="input-group flex-nowrap">
                                <input type="text" value="{{$students->idDetalle}}" name="estudiantes[]" hidden>
                                <input type="text" class="form-control" placeholder="Nota evaluacion" name="notas[]" aria-describedby="addon-wrapping">
                            </div>
                        </td>
                    </tr>
                    @endforeach
            ...
            <div class="text-center mb-3 mt-4" style="width: 100%;">
                <div class="col">
                    <button type="submit" class="btn btn-warning fw-bold">Agregar Calificaciones</button>
                </div>
            </div>
        </form>
    </div>
